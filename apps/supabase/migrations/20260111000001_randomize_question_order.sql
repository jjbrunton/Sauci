...
-- Changes ORDER BY from intensity ASC to RANDOM()
-- Keeps partner-answered priority for gap resolution
...
BEGIN
  ...
  -- ORDER BY: Partner-answered questions first, then RANDOM for variety
  ORDER BY
    EXISTS (
      SELECT 1 FROM public.responses r
      WHERE r.question_id = q.id
