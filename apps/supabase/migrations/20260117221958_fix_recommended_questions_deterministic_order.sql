CREATE OR REPLACE FUNCTION public.get_recommended_questions(target_pack_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(id uuid, text text, partner_text text, is_two_part boolean, pack_id uuid, intensity integer, partner_answered boolean, allowed_couple_genders text[], target_user_genders text[])
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ORDER BY
    ...
    q.intensity ASC,
    q.id ASC;  -- ADDED: Deterministic tiebreaker to ensure both partners see questions in same order
END;
$function$;
