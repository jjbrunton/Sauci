...
The client and checked-in native projects are wired with the real server values
(app id `9a66f3c2-fda7-416f-87e2-6db833b32e9d` and the Sauci code-signing
certificate, SHA-256 fingerprint beginning `DF:C9:17:A9`). A new store build is
required before updates can flow; see [Server registration](#server-registration).
...
than a release step. The checked-in native projects embed the production updates
configuration:

- `ios/Sauci/Supporting/Expo.plist` enables updates and contains the manifest
  URL, runtime version, request headers, code-signing metadata, and public
  certificate.
- `android/app/src/main/AndroidManifest.xml` enables updates and contains the
  same production configuration; `android/app/src/main/res/values/strings.xml`
  supplies its resolved runtime version.

The `expo-updates` config plugin wrote those values from the dynamic config as a
reviewed source change. When future changes intentionally affect native config,
regenerate and review the native diff separately:
...
Remaining before updates flow: cut a new store build from this checked-in native
source. Only builds produced after that point can receive updates.
