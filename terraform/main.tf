...
#
# This manages the configuration for Sauci Supabase projects.
...
#   terraform plan
#
# To import settings for a new workspace:
#   terraform workspace new <name>
#   terraform import supabase_settings.main <project_ref>

# Project settings - API, Auth, Network, Storage configuration
resource "supabase_settings" "main" {
  ...
  auth = jsonencode({
    ...
    # Email templates (see templates/ directory for full HTML)
    ...
    smtp_admin_email   = local.is_production ? "team@send.sauci.app" : null
    smtp_host          = local.is_production ? "smtp.resend.com" : null
    smtp_max_frequency = local.is_production ? 1 : null
    smtp_port          = local.is_production ? "465" : null
    smtp_sender_name   = local.is_production ? "Sauci App" : null
    smtp_user          = local.is_production ? "resend" : null
